...
def is_url_ok_to_follow(url, limiting_domain=None, check=True):
    """
    Inputs:
        url: absolute URL
        limiting domain: domain name
        check: Boolean check or not if in limiting domain, default checks
    ...
    if check:
        if limiting_domain not in url:
            # print("Lim Dom not in url")
            return False
